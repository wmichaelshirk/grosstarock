...
'not_recommend_player_number' => [4],
// 'not_recommend_player_number' => array( 2, 3 ),      // <= example: this
// is not recommended to play this game with 2 or 3 players


// Estimated game duration, in minutes (used only for the launch,
// afterward the real duration is computed)
...
// If in the game, all losers are equal (no score to rank them or explicit in
// the rules that losers are not ranked between them), set this to true
...
// Luck of the game, from 0 (absolutely no luck in this game) to 5 
// (totally luck driven)
'luck' => 3,

// Strategy of the game, from 0 (no strategy can be setup) to 5 (totally 
// based on strategy)
'strategy' => 3,

// Diplomacy of the game, from 0 (no interaction in this game) to 5
// (totally based on interaction and discussion between players)
'diplomacy' => 1,
...
// colors based on player's preferences (see reattributeColorsBasedOnPreferences
// PHP method)
'favorite_colors_support' => true,

// When doing a rematch, the player order is swapped using a "rotation" so 
// the starting player is not the same.  If you want to disable this,
// set this to true
'disable_player_order_swap_on_rematch' => false,

// Game interface width range (pixels)
// Note: game interface = space on the left side, without the column on the 
// right
'game_interface_width' => array(

    // Minimum width
    //  default: 740
    //  maximum possible value: 740 (ie: your game interface should fit with a 
    //   740px width (correspond to a 1024px screen)
    //  minimum possible value: 320 (the lowest value you specify, the better 
    // the display is on mobile)
    'min' => 740,
...
// Game presentation
// Short game presentation text that will appear on the game description page,
// structured as an array of paragraphs.
// Each paragraph must be wrapped with totranslate() for translation and should
// not contain html (plain text without formatting).
...
//  Each tag has a specific ID (ex: 22 for the category "Prototype", 101 for 
//  the tag "Science-fiction theme game")
//  Please see the "Game meta information" entry in the BGA Studio documentation
//  for a full list of available tags:
//  http://en.doc.boardgamearena.com/Game_meta-information:_gameinfos.inc.php
//  IMPORTANT: this list should be ORDERED, with the most important tag first.
//  IMPORTANT: it is mandatory that the FIRST tag is 1, 2, 3 and 4 
// (= game category)
'tags' => [1, 11, 23, 200],
...
// circuit : A plays and choose the next player C, C plays and choose the next 
// player D, ...
